// screens/Analysis/DiseaseDetection/DiseaseHistoryTab.js
...
const DiseaseHistoryTab = ({ toolConfig, newEntry }) => {
  ...
  const loadHistory = async () => {
    try {
      const historyKey = '@analysis_history_disease';
      const archiveKey = '@analysis_archive_disease';
          ...
          onPress: async () => {
            try {
              if (isArchived) {
                ...
                await AsyncStorage.setItem('@analysis_archive_disease', JSON.stringify(updatedArchive));
              } else {
                const updatedHistory = history.filter(item => item.id !== itemId);
                setHistory(updatedHistory);
                await AsyncStorage.setItem('@analysis_history_disease', JSON.stringify(updatedHistory));
              }
            } catch (error) {
  ...
  const handleClearAll = () => {
    Alert.alert(
      'Clear All History',
      `Are you sure you want to delete all ${showArchived ? 'archived' : 'analysis'} history?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Clear All',
          ...
          onPress: async () => {
            try {
              if (showArchived) {
                setArchivedHistory([]);
                await AsyncStorage.removeItem('@analysis_archive_disease');
              } else {
                setHistory([]);
                await AsyncStorage.removeItem('@analysis_history_disease');
              }
            } catch (error) {
            }
          },
