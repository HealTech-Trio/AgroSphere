// screens/Analysis/IrrigationOptimization/IrrigationHistoryTab.js
...
import IrrigationResultScreen from './IrrigationResultScreen';
...
const IrrigationHistoryTab = ({ toolConfig, newEntry }) => {
  ...
  const loadHistory = async () => {
    try {
      ...
      const archiveKey = '@analysis_archive_irrigation';
  ...
  const filterAndSortHistory = () => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(item => 
        ...
        item.crop_data?.name?.toLowerCase().includes(query) ||
        item.result?.status?.toLowerCase().includes(query) ||
  ...
  const handleDeleteItem = async (itemId, isArchived = false) => {
    Alert.alert(
      'Delete Optimization',
      'Are you sure you want to delete this irrigation optimization?',
      [
          ...
          onPress: async () => {
            try {
              if (isArchived) {
                ...
                await AsyncStorage.setItem('@analysis_archive_irrigation', JSON.stringify(updatedArchive));
              } else {
                const updatedHistory = history.filter(item => item.id !== itemId);
                setHistory(updatedHistory);
                await AsyncStorage.setItem('@analysis_history_irrigation', JSON.stringify(updatedHistory));
              }
            } catch (error) {
  ...
  const handleArchiveItem = async (itemId) => {
    try {
      ...
      Alert.alert('Success', 'Optimization moved to archive.');
    } catch (error) {
  ...
  const handleUnarchiveItem = async (itemId) => {
    try {
      ...
      Alert.alert('Success', 'Optimization restored from archive.');
    } catch (error) {
  ...
  const handleClearAll = () => {
    Alert.alert(
      'Clear All History',
      `Are you sure you want to delete all ${showArchived ? 'archived' : 'optimization'} history?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Clear All',
          ...
          onPress: async () => {
            try {
              if (showArchived) {
                setArchivedHistory([]);
                await AsyncStorage.removeItem('@analysis_archive_irrigation');
              } else {
                setHistory([]);
                await AsyncStorage.removeItem('@analysis_history_irrigation');
              }
            } catch (error) {
            }
          },
  ...
  const renderHistoryItem = ({ item }) => {
    const efficiencyScore = item.result?.efficiency_score || 0;
        ...
        >
          <View style={styles.historyCard}>
            <View style={styles.scoreContainer}>
              <View style={[styles.scoreCircle, { borderColor: efficiencyColor }]}>
                <Text style={[styles.scoreText, { color: efficiencyColor }]}>
            ...
            <View style={styles.historyContent}>
              <View style={styles.historyHeader}>
                ...
                {item.result?.status && (
                  <View style={[styles.statusBadge, { backgroundColor: efficiencyColor + '20' }]}>
                    <Text style={[styles.statusText, { color: efficiencyColor }]}>
                      {item.result.status}
                    </Text>
                  </View>
                )}
              ...
              {item.crop_data?.name && (
                <View style={styles.cropRow}>
                  ...
                  <Text style={styles.cropText} numberOfLines={1}>
                    {item.crop_data.name} ({item.crop_data.type})
                  </Text>
                </View>
              )}
              ...
              <View style={styles.historyFooter}>
                ...
                {item.iot_data && (
                  <View style={styles.iotBadge}>
                    <Ionicons name="hardware-chip" size={14} color="#FF9800" />
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyTitle}>
        {showArchived ? 'No Archived Optimizations' : 'No Optimization History'}
      </Text>
      <Text style={styles.emptyDescription}>
        {searchQuery 
          ? 'No results found for your search'
          : showArchived 
            ? 'Archived irrigation optimizations will appear here'
            : 'Your irrigation optimization history will appear here after you complete your first analysis'
        }
      </Text>
      ...
      <View style={styles.controlsContainer}>
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            placeholder={`Search ${showArchived ? 'archived' : 'optimization'} results...`}
            value={searchQuery}
...
export default IrrigationHistoryTab;